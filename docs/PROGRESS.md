...
on `main` and push, no per-milestone branch dance. Tags on `main` still
mark milestones — don't tag `m11` / `m12` until visual verification
confirms the render in Figma.
...
- `2026-04-22` — Two designer-usability fixes landed on `main` after M11. (1) SVG path normaliser: the old whitespace-regex tokenizer broke on SVGO-compacted paths (`0-2.53`, `.4.07.55`, `1e-3`) and let `A` / `a` elliptical-arc commands through verbatim — Figma's vector parser rejects both. New `tokenizeSvgPath` walks the `d` attribute with a proper command / number regex; new `arcToCubics` lowers every arc to one or more cubic Bézier segments via the SVG-spec endpoint-to-center parameterization (segments split on π/2 boundaries). 7 new tests in misc-fidelity.test.ts; LIMITATIONS #16 updated. (2) Auto-layout wrap fix: `flex-wrap: wrap` mapped to `layoutWrap: 'WRAP'` but `counterAxisSizingMode` stayed `'FIXED'`, which Figma silently treats as "ignore wrap" — card grids collapsed to one tall row. WRAP containers now emit `counterAxisSizingMode: 'AUTO'`; non-wrap containers still emit FIXED. 2 new tests in auto-layout.test.ts; `card-grid` + `flex-wrap` snapshots refreshed. 302 workspace tests green (CLI 284, IR 17, plugin 1).
...
- `2026-04-22` — M12 "landing dogfood — section by section" kicked off. Workflow: carve each section of `examples/landing/source/index.html` into its own standalone HTML under `examples/landing/sections/`, run `convert --hydrate --viewport 1440x900 --font-fallback Inter`, paste IR into the Figma plugin, compare against the browser render, fix whatever's broken in the CLI / IR / plugin, re-paste. Nav shipped 2026-04-22 in 3 trunk commits (cascade: `inherit` keyword + `place-*` shorthand expansion; native SVG import via `figma.createNodeFromSvg` replacing the path-reconstruction fallback; nav.html fixture). Hero shipped 2026-04-22 in 3 more trunk commits — mixed-style text runs for `<em>` inside `<h1>` (two-tone italic accent), layout sizing fixes (explicit-cross-axis stretch guard, `%` widths, border subtraction, fr-track flooring), `data-c2f="snapshot"` opt-in so decorative regions like `.hero-visual` bypass the Frame-tree reconstruction and come through as single Chromium-rendered PNGs. Snapshot pipeline refined once more for visual overflow (union-of-descendants bounds so rotated absolute children don't clip) + robust base64 decoding in plugin (atob-independent). Next: skip carving more sections individually, paste the whole landing page IR and catch the rest in one pass. 284 CLI / 302 workspace tests green.
